...
$(".js-slider").slick({
  ...
  autoplaySpeed: 3000,
  prevArrow:
    '<button class="slider__prev"><i class="uk-icon-angle-left"></i></button>',
  nextArrow:
    '<button class="slider__next"><i class="uk-icon-angle-right"></i></button>',
  dots: true,
});
...
$(".js-insert-file").each(function (i, wrapper) {
  ...
  $wrapper
    .find(".ms2galleryform-file-image-wrapper")
    .append("<span>Вставить</span>");
  $wrapper.on("click", ".ms2galleryform-file-link", function () {
    ...
    $.markItUp({
      ...
      replaceWith:
        '<a href="' +
        link +
        '" class="comment-image" target="_blank"><img src="' +
        link +
        '" alt="' +
        name +
        '"></a>',
          ...
          $.ajax({
            ...
            success: function (response) {
              ...
              if (response.redirect) {
                ...
                UIkit.notify(
                  "<b>Заказ успешно оформлен.</b><br> Наши менеджеры свяжутся с Вами.",
                  {
                    status: "success",
                  },
                );
...
$("#cart-tabs").on("show.uk.switcher", function (event, area) {
  if (
    $(miniShop2.Order.paymentInput, miniShop2.Order.order).filter(
      ":visible:checked",
    ).length == 0
  ) {
    $(miniShop2.Order.paymentInput, miniShop2.Order.order)
 ...
 **/
var switcherContactsTabsActive = $("#switcher-contacts-content>li")
  .has("li.active")
  .index();
...
$(".ajax_form").append(
  '<input type="text" name="org" value="" class="_org" style="position:absolute; visibility:hidden; height: 0; width: 0; padding: 0; border:none;"/>',
);
...
function optionsWidth() {
  ...
  groups.each(function (i1, group) {
    ...
    if (minWidth > 0) {
      ...
      let normalPerRow = Math.ceil(
        items.length / Math.ceil(items.length / perRow),
      );
  ...
  $(".js-calc").each(function (i, form) {
    ...
    const calculate = function (e) {
      ...
      let payment = payments.find(
        (row) => row.id === parseInt(paymentSelect.value),
      );
      let delivery = deliveries.find(
        (row) => row.id === parseInt(deliverySelect.value),
      );
        ...
        if (delivery.id === 4) {
          ...
          if (payment.id === 1) {
            ...
            if (costInput.value < 5000) {
              ...
              resultPrice.innerHTML =
                "450 + " +
                costInput.value * 0.08 +
                " = " +
                (costInput.value * 0.08 + 450) +
                " руб.";
            } else if (costInput.value >= 5000 && costInput.value < 12000) {
              resultTime.innerHTML = "4-6 дней";
              resultPrice.innerHTML =
...
WebFont.load({
  custom: {
    ...
    urls: [
      "/assets/nemopro/src/fonts/sf/SFUIText.css",
      "/assets/nemopro/src/fonts/sf/SFUIDisplay.css",
    ],
  },
  active: function () {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll("a.product-rating").forEach((link) => {
